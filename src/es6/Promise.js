...
// 자바스크립트는 기본적으로 1스레드 비동기 처리 방식을 사용하고 있음
// 자바스크립트에서 순차적으로 실행을 하고 싶을 경우 콜백함수를 이용하여야 함
// 순차적으로 실행할 것이 많아지게 되면 콜백지옥이라고 불리는 형태가 만들어지게 됨
// 프로미스는 이러한 콜백지옥을 해결하기 위해서 사용하는 객체임

// 프로미스에는 pending, fulfilled, rejected 3가지 상태가 존재함
// pending : 대기상태. 비동기 함수가 실행되고 아직 처리가 완료되지 않은 상태
// fulfilled : 완료상태. 함수가 실행된 후 정상적으로 처리가 완료된 상태
...
// 사용법 :
// 선언 :   function 프로미스를 사용할 함수명(매개변수) {
